...
public class NonAWSCondition implements Condition {
    @Override
    ...
    static boolean check(final String[] activeProfiles) {
        return Stream.of(activeProfiles)
                .anyMatch(p -> TEST_PROFILE.equals(p) || LOCAL_PROFILE.equals(p))
                && Stream.of(activeProfiles).noneMatch(pr -> KMS_PROFILE.equals(pr));
    }
}
